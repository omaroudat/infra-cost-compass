        ...
        try {
          switch (selectedTable) {
            ...
            case 'wirs':
              ...
              // Map Excel status values to database enum values
              const mapStatus = (excelStatus: string): WIRStatus => {
                if (!excelStatus) return 'submitted';
                const status = excelStatus.toLowerCase();
                if (status.includes('approved') || status.includes('completed') || status === 'a') {
                  return 'completed';
                }
                return 'submitted';
              };

              // Map Excel result values to database enum values  
              const mapResult = (excelResult: string): WIRResult | undefined => {
                if (!excelResult) return undefined;
                const result = excelResult.toLowerCase();
                if (result.includes('a') || result.includes('approved')) return 'A';
                if (result.includes('b') || result.includes('conditional')) return 'B';
                if (result.includes('c') || result.includes('rejected')) return 'C';
                return undefined;
              };
              ...
              await addWIR({
                ...
                status: mapStatus(wirRow.status || ''),
                result: mapResult(wirRow.result || ''),
